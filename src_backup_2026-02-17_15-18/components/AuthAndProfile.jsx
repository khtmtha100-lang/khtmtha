...
// --- 1. المكون الأساسي: الزر اللمسي (TactileButton) ---
      ...
      <div className={`absolute top-full right-0 mt-3 w-64 p-3 rounded-[24px] border-2 border-b-4 shadow-xl z-50 animate-fade-in-up ${isDarkMode ? 'bg-[#2A2640] border-[#3E3859]' : 'bg-white border-[#E2E8F0]'}`}>
        ...
        <div className={`flex items-center gap-3 p-3 mb-2 rounded-2xl ${isDarkMode ? 'bg-black/20' : 'bg-slate-50'}`}>
          ...
          <div>
            ...
            <span className={`text-[10px] font-bold flex items-center gap-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              {userAge && userGovernorate && (
                <>
                  {userAge} • {userGovernorate}
                </>
              )}
        ...
        <button 
          ...
          className={`w-full p-3 rounded-xl flex items-center gap-3 hover:bg-black/5 dark:hover:bg-white/5 font-bold text-sm mb-1 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-800'}`}
        >
          {isDarkMode ? (
        ...
        <button 
          onClick={() => { toggleFullscreen(); onClose(); }} 
          className={`w-full p-3 rounded-xl flex items-center gap-3 hover:bg-black/5 dark:hover:bg-white/5 font-bold text-sm mb-1 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-800'}`}
        >
          {isFullscreen ? (
        ...
        <div className={`h-px ${isDarkMode ? 'bg-slate-700' : 'bg-slate-100'} my-2`}></div>
        
        <button 
          onClick={onLogout} 
          className={`w-full p-3 rounded-xl flex items-center gap-3 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 font-bold text-sm text-red-500 transition-colors`}
        >
...
// --- 3. واجهة تسجيل الدخول (LoginView) ---
export const LoginView = ({ isDarkMode, onLoginSuccess }) => {
  ...
  const governorates = [
    ...
    "الأنبار", "ديالى", "ذي قار", "بابل", "واسط", "ميسان", "القادسية", 
    "المثنى", "صلاح الدين", "دهوك", "السليمانية"
  ];
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-5 py-10 w-full animate-fade-in-up">
      <div className={`text-center mb-8 relative z-10 transition-all duration-500 ${step > 0 ? 'scale-75 mb-2' : ''}`}>
        ...
        <h1 className={`text-4xl md:text-5xl font-black tracking-tighter mb-1 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
          ختمتها<span className="text-yellow-500">.</span>
        </h1>
        <p className={`text-xs md:text-sm font-bold opacity-60 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
          الطريق الأمتع للـ 100
        </p>
      </div>
      ...
      <div className={`w-full max-w-sm relative z-20`}>
          ...
          <div className="animate-fade-in-up">
            <div className={`p-6 rounded-[32px] border-2 shadow-2xl backdrop-blur-lg mb-6 ${isDarkMode ? 'bg-slate-800/80 border-slate-700' : 'bg-white/90 border-white'}`}>
              ...
              <button 
                ...
                className="w-full py-3 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 text-slate-500 dark:text-slate-400 font-bold hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-sm"
              >
        ...
        {step > 0 && (
          <div className="animate-slide-up bg-white dark:bg-slate-800 p-6 rounded-[2rem] border-2 border-slate-100 dark:border-slate-700 shadow-xl relative">
            {step === 1 && (
              <>
                <h3 className={`text-xl font-black text-center mb-6 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
                  أهلاً! شنو اسمك؟ 👋
                </h3>
                <input 
                  type="text" 
              ...
              <>
                ...
                <div className="flex gap-3">
                  <select 
                    ...
                    value={formData.age} 
                    onChange={handleInputChange} 
                    className={`${inputClasses} flex-[0.6] appearance-none cursor-pointer`}
                  >
            ...
            <div className="flex gap-3 mt-6">
              <button 
                ...
                className="flex-[0.3] p-4 rounded-xl font-bold text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
                رجوع
              </button>
              <TactileButton 
